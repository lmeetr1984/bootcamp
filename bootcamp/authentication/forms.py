    ...
    if '*' not in ALLOWED_SIGNUP_DOMAINS:
        try:
            ...
            if domain not in ALLOWED_SIGNUP_DOMAINS:
                # 校验失败，要用raise 一个error
                raise ValidationError(
                    u'Invalid domain. Allowed domains on this network: {0}'.format(','.join(ALLOWED_SIGNUP_DOMAINS)))
...
class SignUpForm(forms.ModelForm):
    # 这是一个ModelForm，字段的定义，限制同model

    # 定义字段，注意定义了widget： 这个widget指明了如何使用html元素创建字段
